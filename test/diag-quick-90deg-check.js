...
app.whenReady().then(async () => {
  ...
  for (const speciesKey of ['cat_a', 'dog_corgi']) {
    ...
    const neckRgb = hexToRgb(collars[0].hex);
    console.log(`${speciesKey} @ 90deg: neck collar ${collars[0].hex} present = ${colorPresent(dump.buf, neckRgb)}`);
  }
  app.quit();
});
